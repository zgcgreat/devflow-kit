...
Each skill is a Markdown file (`_SKILL.md`) that describes a specific engineering workflow. When loaded into an agent's context, the agent follows the workflow — including verification steps, anti-patterns to avoid, and exit criteria.
...
Browse the `skills/` directory. Each subdirectory contains a `_SKILL.md` with:
...
Copy the relevant `_SKILL.md` content into your agent's system prompt, rules file, or conversation. The most common approaches:
