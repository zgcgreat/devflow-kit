...
> - **reference（前端项目）**：`references/reference/ui-anti-patterns.md`（可全读，111 行）
...
[ ] .devflow-kit/<req-id>/02a-ui-design.md — 前端项目必须
```
...
- `@.devflow-kit/<req-id>/02-design.md`（如有）
- `@.devflow-kit/<req-id>/02a-ui-design.md`（如是前端项目）
- `@.devflow-kit/<req-id>/03-tasks.md`
- `@.devflow-kit/<req-id>/05-test-report.md`
- 本次变更的 git diff（用户提供或 AI 通过工具获取）
- `@devflow-kit/references/reference/ui-anti-patterns.md`（如是前端项目）

## 你的职责

**⚠️ 强制要求**：必须严格按照 `@devflow-kit/templates/06-code-review.md` 模板的完整结构分四轮审查（后端 / lib 项目跳过第三轮），**保存到 `.devflow-kit/<req-id>/06-code-review.md`**。**不得省略或改写任何段落**。
...
```
UI 审查触发判定：
- 存在 02a-ui-design.md：是/否
- diff 涉及 UI 文件：是/否 — 列出具体文件
→ 触发结果：执行/跳过
```

**触发条件**（必须同时满足）：
1. 本次需求 含 `02a-ui-design.md`
2. **且** diff 涉及任何 UI 文件（`.css` / `.tsx` / `.vue` / `.html` / `.svelte` / `tailwind.config.*` 等）

**跳过条件**：
- 不存在 `02a-ui-design.md` → 跳过（后端 / CLI / lib 项目）
- 存在 `02a-ui-design.md` 但 diff 不涉及 UI 文件（如纯后端改动）→ 跳过，输出"本次 diff 无 UI 变更"

**⚠️ 强制读取**：执行前必须读取 `02a-ui-design.md` 和 `ui-anti-patterns.md`。
...
- ⏳ 实现里的颜色值是否全部来自 02a-ui-design.md frontmatter（CSS variables / theme）？
- ⏳ 是否有硬编码的 hex / 字号 / 间距数值？（命中即 🔴 Critical）
- ⏳ 字体是否与 02a-ui-design.md 声明一致？是否引入了 anti-pattern 字体（Inter / Roboto / Arial）？
...
逐项对照 `@devflow-kit/references/reference/ui-anti-patterns.md` 的"强制禁忌"段：
...
**⚠️ 强制输出**：必须先读取 `02a-ui-design.md` 第 1 节"美学北极星"。

**输出格式**：

```markdown
### 3.3 视觉北极星一致性
- 设计调性声明：<从 02a-ui-design.md 提取>
- 实现调性评估：<AI 判断>
- 一致性结论：一致/部分一致/不一致
- 失焦点（如有）：<具体说明>
- 修复建议（如有）：<具体说明>
...
回到 02a-ui-design.md 第 1 节"美学北极星"，问一个问题：
...
- 🟢 Monitored · 仅记录不处理 → lessons-learned.md
...
审查统计：
- 四轮审查: <列出每轮状态>
- 发现问题: <N> 个（Critical: <M> / Major: <K> / Minor: <L>）
- 已接受: <列出已接受的 Critical 或"无">

➡️ 下一步：进入集成阶段；如存在 Critical / Blocked 项，将先暂停处理。
```

> **执行阶段**：审查是执行阶段，无阻塞且下一步明确时可继续进入下一阶段。

## 触发下一步

- 有 Critical 待修 → `@devflow-kit/references/prompts/4-dev.md`（执行 fix 任务）
- 全部通过或人工接受 → `@devflow-kit/references/prompts/7-integration.md`
