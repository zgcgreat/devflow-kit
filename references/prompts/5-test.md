...
> - **reference（按节读取，禁止整读）**：`references/reference/test-pyramid.md` § 适用矩阵 + 需要的那几轮详情


> **核心思想**：测试从 AC 和风险派生，不从实现派生。功能覆盖必做；性能、安全、兼容、可观测按风险触发，跳过必须有理由。
> 本 prompt 先确定风险驱动测试矩阵；具体工具和标准按需查 `@devflow-kit/references/reference/test-pyramid.md`。
...
- 各任务的 `*-dev-log.md`
- 已存在的测试代码
- `@devflow-kit/references/reference/test-pyramid.md`（5 轮的工具 / 标准 / 清单）
...
|---|---|
| 任何行为变化 | 功能测试 + AC 覆盖 |
| UI / 页面 / 用户交互 | 功能测试 + 浏览器手测 / 可访问性基础检查 |
| API / 数据流 / 外部集成 | 功能测试 + 集成测试 + 错误路径 |
| 性能敏感、列表/查询/渲染路径 | 性能测试 |
| 鉴权、授权、支付、PII、secret、依赖升级 | 安全测试 |
| schema / migration / 公共 API | 兼容性 / 迁移测试 |
| 后台任务、分布式调用、生产发布 | 可观测性验证 |
...
**禁止**没声明就跳过风险触发的轮次。每个 ❌ 必须有理由；每个 ⚠️ 必须说明裁剪范围。
...
**前置**：本任务在 4-dev 步骤 1.7 已生成迁移文件（如未生成，回退到 dev 阶段——R4.5 违规）。在 dev-log.md「数据库迁移」段 trace 文件路径，本步对这些文件做端到端验证。

- ⏳ 迁移文件路径已 trace（来自 `<task-id>-dev-log.md`「数据库迁移」段）
...
- `.devflow-kit/<req-id>/05-test-report.md`（**⚠️ 强制要求**：必须严格按 `@devflow-kit/templates/05-test-report.md` 模板的完整结构输出，含风险驱动测试矩阵、真实命令输出、UAT 脚本和测试质量自检。**不得省略或改写任何段落**。）
...
- **R5.2**：bug 修复必伴随回归测试，加入 `.devflow-kit/lessons-learned.md`
...
➡️ 下一步：进入审查阶段；如测试失败或证据不足，将先回到开发阶段修复。
```

> **执行阶段**：测试是执行阶段，无阻塞且下一步明确时可继续进入下一阶段。

## 触发下一步

- 任何轮次发现 🔴 问题 → 回到 `@devflow-kit/references/prompts/4-dev.md` 修复（产 fix 任务）
- 全部通过 → `@devflow-kit/references/prompts/6-review.md`（review 阶段会检查风险触发的测试是否都有证据）
