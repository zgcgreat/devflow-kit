...
> 用法：`@devflow-kit/references/prompts/L-restyle.md` + 一句话目标
...
- ❌ 全新项目（用 `0-confirm` → `2a-ui-design`）
- ❌ 业务功能变化（视觉变 + 功能变 = 拆成两个独立 req）
...
- 项目里已有的 `02a-ui-design.md`（如有，作为 v1 调性来源）
- `@devflow-kit/references/reference/ui-aesthetics.md`（调性卡片）
- `@devflow-kit/references/reference/ui-anti-patterns.md`
...
**优先**：读 `.devflow-kit/archive/.../02a-ui-design.md` 或仓库里现存的 02a-ui-design.md，提取调性字段
**降级**（无 UI-设计）：从代码反向提取
...
任何"动"列发现要触及"不动"列的内容 → 立即停下，按 R7.1 开新需求 处理。

### 5. 写新 02a-ui-design.md（标 v2）

**⚠️ 强制要求**：必须严格按照 `@devflow-kit/templates/02a-ui-design.md` 模板的完整结构输出。**不得省略或改写任何段落**。
- 路径：`.devflow-kit/<req-id>/02a-ui-design.md`
- 在 frontmatter 加：
  ```yaml
  version: 2
  supersedes: archive/<old-req-id>/02a-ui-design.md
...
- **R7.1**（关键）：发现要改业务逻辑 / 接口 / 路由 → **立即停下**，开新需求。Restyle 绝不允许成为业务变更的偷渡口
...
- ⏳ 新调性已选定，用户已知代价
- ⏳ 影响面"动 / 不动"清单完整
- ⏳ 02a-ui-design.md v2 已写，组件接口与 v1 完全一致
...
- 用户确认 → `@devflow-kit/references/prompts/3-task.md`（按上述批次落实任务）
- → `@devflow-kit/references/prompts/4-dev.md`（每任务 fresh context，UI 任务自动扫 anti-patterns）
- → `@devflow-kit/references/prompts/6-review.md`（**第三轮 UI 视觉审查必跑**，含 v1 vs v2 对比段）
- → `@devflow-kit/references/prompts/7-integration.md`（UAT 重点：老用户能否完成关键流程；视觉是否传达声明的调性）
