...
> - **reference**：
>   - `references/reference/ui-anti-patterns.md`（可全读，111 行）
>   - `references/reference/ui-aesthetics.md` § 调性（按节读取）
...
- `@.devflow-kit/<id>/00-requirements.md`（**必读**，含 0-confirm 阶段已选的「视觉调性」）
- `@.devflow-kit/<id>/01-analysis.md`
- `@.devflow-kit/<id>/02-design.md`（**必读 `## 0. 技术栈选定` 段**——字体 / UI 库 / 组件库 / 图标库的选择必须基于这个栈）
- `@.devflow-kit/CONTEXT.md`
- `@devflow-kit/references/reference/ui-aesthetics.md`（决策框架）
- `@devflow-kit/references/reference/ui-anti-patterns.md`（反 AI-slop 清单）
...
- **greenfield**：空项目 / 第一个 UI req / 用户明说"重新做 UI" → 跳过 1.5，直接进 1
- **brownfield**：已存在 UI，本次需求 是加新页面 / 新组件 / 改现有页面 → **必走 1.5 视觉语汇对齐**

判定信号：`src/` 下已有 `.css` / `.scss` / `.tsx` / `theme` / `design-tokens` 文件，或 `00-requirements.md` 的视觉调性段标为"沿用现有"。
...
- **目的**：界面解决什么问题？谁用？核心动作？
- **调性**：**从 `00-requirements.md` 的「视觉调性」段读取已选项**（在 0-confirm 步骤 0.6 已确定）。**不允许在本阶段让用户重选**。
  - 如果 00-requirements.md 没有该字段（旧项目 / 跳过 0-confirm 直接进 2a）→ 才按 `ui-aesthetics.md` 的「调性」节展示卡片让用户选
  - 如果用户在本阶段主动说"想换调性" → 引导回 0-confirm 改 00-requirements.md，不在 2a 内部改（避免与原始决策不一致）
...
#### 1.5.3 写入 02a-ui-design.md

在 `02a-ui-design.md` 的 `## 0. 视觉语汇对齐`（新增段，仅 brownfield 填）贴观察报告 + 用户校准结论。

### 2. 美学维度决策

按 `ui-aesthetics.md` 的 5 个维度逐项决策。每个决策必须给理由。**优先调用外部 skill 拿候选**，没装才回退到内置经验：

| 维度 | 装了 ui-ux-pro-max | 没装时的内置基线 |
|---|---|---|
...
- 没装 uipro → 在该维度开头明确写 `(uipro 未检出，使用内置基线)`，避免后期审查混淆来源。内置基线选型参考：
  - **字体**：出版物→IBM Plex / 技术→Space Grotesk+JetBrains / 创意→Plus Jakarta+Sans / 电商→Noto Sans
  - **颜色**：专业→蓝色系(oklch 0.6 0.15 260) / 温暖→橙红系(0.65 0.18 30) / 创意→紫红系(0.6 0.2 340)
  - **图表**：5 色有序序列用相邻 hue、分类用均匀分布 hue、强调用补充色
  - **UX**：表单有 label、按钮语义明确、错误提示可见、空态有引导
...
> 写一版完整 02a-ui-design.md 之前，先给 30 秒可看的 v0。方向错了此时换代价最低。
...
- **说"可以 go"** → 进步骤 4，写完整 02a-ui-design.md（保存到 `.devflow-kit/<id>/02a-ui-design.md`）
- **说"某维度偏了"** → 仅改该维，重贴 v0（别改其他维）
- **说"全推了重来"** → 回步骤 1–步骤 2，不是再翻新花样

#### 3.4 进入步骤 4 前写入 02a-ui-design.md

在 02a-ui-design.md 顶部的 `## 1. 美学北极星` 后追加一段 `### v0 确认摸路`，记录：
...
强制约定（写入 02a-ui-design.md 的占位符策略段）：
...
| 客户推荐墙 | 明标「推荐语 PLACEHOLDER」 | 编用户名 + 照片 + 评论 |
...
输出 02a-ui-design.md 前，**逐条对照** `ui-anti-patterns.md` 的"强制禁忌"段，确认本设计未命中任何一条。命中的必须改或显式标注例外理由。

## 输出

- `.devflow-kit/<id>/02a-ui-design.md`（**⚠️ 强制要求**：必须严格按 `@devflow-kit/templates/02a-ui-design.md` 模板的完整结构输出。**不得省略或改写任何段落**。）
...
- **更新 `.devflow-kit/STATE.md`**：
  ...
  - `上次完成阶段` 设为 `ui-design`
  - `下一阶段` 设为 `task`
  - 在「阶段进度」清单中打钩 `2a UI设计 → 02a-ui-design.md`
...
- **步骤 1**（美学方向）：调性已从 00-requirements.md 读到，可补 `--domain product` 拿场景模式
...
- 步骤 4-5 仍按本 prompt 执行（impeccable 不直接产 02a-ui-design.md）
...
- **R10.1**：颜色必须 OKLCH（除非有遗留约束并显式声明）
- **R10.2**：字体必须避开 AI slop 默认词；用了的话必须解释为什么不能换
- **R10.3**：每个美学决策必须给理由（"为什么选这个调性 / 这个 hue / 这个字体"），不允许"看起来比较好"
- **R10.4**：02a-ui-design.md 输出前必须 grep `ui-anti-patterns.md` 完成自检
- **R10.5**：brownfield 必走 1.5，视觉语汇对齐报告必须经用户确认后才能进步骤 2
- **R10.6**：v0 草稿必须在写完整 02a-ui-design.md 前给用户确认（除非是极小改动，且用户明说"别 v0 直接写"）
- **R10.7**：禁止编造数据 / 好评 / logo；缺素材走占位符策略（7 节表格）
...
```
✅ 2a UI设计 完成
📝 产物：.devflow-kit/<req-id>/02a-ui-design.md
📊 STATE.md 阶段进度已更新：[x] 2a UI设计 → 02a-ui-design.md
...
> ⚠️ **关键阶段**：2a UI设计是关键阶段，必须等用户回复后才能进入下一阶段。

## 触发下一步

`@devflow-kit/references/prompts/3-task.md` —— 拆任务时把 design tokens 物化到代码（CSS variables / theme 文件）作为第一批任务
