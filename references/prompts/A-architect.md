# 横向命令 · A-architect — 建立 / 重构项目级 system-architecture.md

> ⚠️ **进入本阶段前，必须先加载**：`devflow-kit/agent-skills/skills/planning-and-context/_SKILL.md`

> **触发方式**：`@devflow-kit/references/GO.md` + `建立架构 / 架构梳理 / 重构架构 / architect / 重审 ADR`
> 不属于任何 req，不写 00-requirements.md。直接产出 / 重构 `.devflow-kit/system-architecture.md`。
...
- **首次建立**：项目从未跑过 A-architect，CONTEXT.md 已存在但 system-architecture.md 缺失
...
| 工作流 | 干什么 | 何时跑 | 改 system-architecture.md 哪段 |
|---|---|---|---|
| **A-architect**（本文）| 重写 / 大改 ARCHITECTURE 全篇 | 首次建立 / 重大重构 / ADR 重审 | 任何段都可改，含模块清单、依赖规则、ADR 状态变更 |
| **A-evolve** | 把 需求级沉淀**单点 append** 到 ARCHITECTURE | 每月 / 每季 批量 | 仅 append ADR 列表 / 跨模块契约段 / 修订历史 |
...
- `@.devflow-kit/CONTEXT.md`（必读 · 技术栈 + 既有抽象 + 禁动清单）
- `@.devflow-kit/system-architecture.md`（如有，则是重构跑；如无，则是首跑）
- `@.devflow-kit/需求LOG.md`（看历史 req 演进轨迹）
- `@.devflow-kit/archive/*/02-design.md`（按需 · 重构跑时抽样几份）
- 仓库代码（必读 · `src/` 顶层结构 + `package.json` / 等价依赖文件）
- `@devflow-kit/templates/system-architecture.md`（结构模板）
...
读 `.devflow-kit/system-architecture.md`：
...
✅ 已有 system-architecture.md：<是 / 否，N 行，最近修订 YYYY-MM-DD>
...
写入 ARCHITECTURE § 2.2。**违例 → 列出文件，但不在本工作流修，让用户决定开 fix req 还是接受现状**。

### 步骤 4 · ADR 列表（首跑 / 重构都必跑）

#### 4.1 首跑：从 上下文「已锁技术决策」+ 代码扫描提取
...
按 ARCHITECTURE 模板 § 3 的 7 字段格式（状态 / 取舍 / 决定 / 理由 / 代价 / 来源 req / 推翻成本）填齐。
...
**首跑**：用 `write_to_file` 创建 `.devflow-kit/system-architecture.md`，**⚠️ 强制要求**：必须严格按 `@devflow-kit/templates/system-architecture.md` 模板的完整结构输出，按模板 8 段全填（不适用的段可留 N/A 但**不能删段**——后续 A-evolve 要按段 append）。**不得省略或改写任何段落**。

**重构**：

- 先备份：`cp .devflow-kit/system-architecture.md .devflow-kit/system-architecture.md.bak-<YYYY-MM-DD>`
...
如果本次 ADR 变动有"AI 实施层影响"（比如 ADR-002 缓存改 Redis → 上下文 的「已锁技术决策」段也要同步），**问用户**：
...
- `.devflow-kit/system-architecture.md`（必产，**必须严格按 `@devflow-kit/templates/system-architecture.md` 模板完整结构输出**）
- `.devflow-kit/system-architecture.md.bak-<date>`（重构跑必产）
- 0~1 次对 `.devflow-kit/CONTEXT.md` 的同步 patch（仅当步骤 7.3 用户选 1）
- 在 `STATE.md` 写 `last_architect_at: <YYYY-MM-DD>`

## 约束

- **不动业务代码**：本工作流仅写 `.devflow-kit/` 内文件，禁动 `src/`
...
- **上下文 同步可选**：步骤 7.3 是 opt-in，避免悄悄改 上下文 引发 4-dev 行为漂移
...
- 首跑完成 → 提示"已建立项目级架构文档。后续 需求的 2-design 阶段会读它。下次大架构调整再跑本工作流"
- 重构完成 → 提示"修订已写入。如有 ADR 变化影响 AI 实施层，记得跑 `@A-evolve` 同步到 上下文"
- 检测到 §3.2 模块违例 ≥ 5 → 建议用户开 fix req（走 0-confirm）专门修
