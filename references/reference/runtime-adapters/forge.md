...
- 没装 Forge：flow-kit 行为不变，继续按 `@devflow-kit/references/GO.md` 和 `prompts/*.md` 使用。
- 装了 Forge：AI 可以把 flow-kit 的阶段、req-id、task-id 和风险信息写入 Forge state/routing，供 runtime guard 使用。
...
devflow-kit/templates/03-tasks.md
...
|---|---|
| `0-confirm` | 记录新需求、风险、是否需要完整流程 |
| `1-analysis` | 标记需求产物生成中，禁止直接跳到实现 |
