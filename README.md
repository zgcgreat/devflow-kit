...
> **🆕 v2.3 新特性**: 前置产物全量读取框架 + 智能降级策略 + 完整性自检 [查看升级说明](docs/CHANGELOG_v2.0.md)
...
每个 stage 都会先扫描所有可能的前置产物，根据实际存在情况决定读取策略：

- **🔴 必须读取**：缺失会阻塞流程（如 01-需求分析.md 的 AC 列表）
- **🟡 建议读取**：缺失采用降级策略（如上下文.md 缺失时从 package.json 推断技术栈）
- **🟢 可选读取**：补充信息（如经验总结.md 提供历史教训）

### 2. 智能降级策略

当某个产物不存在时，AI 不会报错停止，而是采用合理的替代方案：
...
每个 stage 结束时都会进行自检，确保充分利用了所有可用信息：
...
**优势**：
- ✅ **灵活性**：能处理各种异常情况（产物缺失、顺序错乱）
- ✅ **容错性**：每个 stage 都有明确的降级策略
- ✅ **透明度**：AI 向用户展示完整的思考过程
- ✅ **质量保证**：完整性自检确保不遗漏关键信息
