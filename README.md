...
| **05-测试报告.md** | 第六轮：五轮测试（功能/性能/安全/兼容/可观测）报告 | 你会看到，可以纠偏 |
...
Fast 模式：修复 settings 页面按钮文案里的 typo。
```

```text
@devflow-kit/flow/GO.md

修复用户点击"保存"后有时不跳转的 bug。
```
...
AI 会：反问澄清 → 写需求确认书 → 整理需求和验收准则 → 技术设计 → 拆任务 → 逐任务实现（TDD）→ 测试 → Review。每个阶段的产物存到 `.specs/<req-id>/`。
...
Strict 模式：把用户 session 存储从 Redis DB 0 迁移到带 namespace 的 Redis cluster，要有回滚计划。
```
...
我要在 Redis 和 Postgres 之间选一个做消息队列，帮我做技术设计。
```

```text
@devflow-kit/flow/GO.md

梳理一下当前项目的模块结构，建立架构文档。
```
...
设计一个极简风格的仪表盘页面，定义 design tokens。
```

```text
@devflow-kit/flow/GO.md

对当前首页做视觉 audit，找出 AI 生成 UI 的常见问题。
```
...
AI 检测到没有 `上下文.md` 后会先问你是否扫描项目，扫描完再进入需求阶段。
