...
收到 01-analysis.md 后，AI **必须**展示如下精简格式（每张卡只露 1 行后端 + 1 行前端 + 适合）；用户选定后再展开完整工程清单：
...
📌 我对你的「<项目名>」的初步推荐：
   - 首选：<编号><名称> — 因为 <一句话理由，结合 ANALYSIS 的 AC + 非功能性需求 + 团队画像>
   - 备选：<编号><名称> — 因为 <理由>
...
- 推荐理由必须**结合 01-analysis.md 的 AC + 非功能性需求 + CONTEXT.md 的团队偏好**（QPS / 团队语言栈 / 部署环境 / 数据规模 / 是否国内）
...
1. 写入 `02-design.md` 的 `## 0. 技术栈选定` 段（结构见末尾）
...
6. **变栈** = 改 02-design.md 的 0 段并全文复审 → 视为开启新需求处理（R7.1）

### `## 0. 技术栈选定` 段结构（写进 02-design.md）
