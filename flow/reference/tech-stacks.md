...
收到 01-需求分析.md 后，AI **必须**展示如下精简格式（每张卡只露 1 行后端 + 1 行前端 + 适合）；用户选定后再展开完整工程清单：
...
- 推荐理由必须**结合 01-需求分析.md 的 AC + 非功能性需求 + 上下文.md 的团队偏好**（QPS / 团队语言栈 / 部署环境 / 数据规模 / 是否国内）
...
1. 写入 `02-方案设计.md` 的 `## 0. 技术栈选定` 段（结构见末尾）
...
6. **变栈** = 改 02-方案设计.md 的 0 段并全文复审 → 视为开启新需求处理（R7.1）

### `## 0. 技术栈选定` 段结构（写进 02-方案设计.md）
