...
### 1.4.2 写入 开发记录.md「6 维自查」段

每条 grep **必须**贴入 `<task-id>-开发记录.md` 的「6 维自查」段：
...
4. 对每条命中的"强制禁忌"，在执行计划里**显式声明**：
   ...
   - 「已知 X 是禁忌，本任务采用 Y 替代」
5. **Token 来源单一**：颜色 / 字体 / 间距 / 圆角 / 动效必须从 02a-UI设计.md frontmatter 派生的 CSS variables / theme 文件中取。**禁止**在组件代码里硬编码颜色或字号（frontend-rules 第 2.1 节）
6. **React 三条硬规则马上写入计划**（仅 React 任务）：禁 `const styles` / 跨文件用 `Object.assign(window, ...)` / 禁 `scrollIntoView`（frontend-rules 第 1.1–1.3）
7. 实现完成后再扫一遍 anti-patterns + frontend-rules 第 10 节交付清单（self-review），写入 开发记录.md

> 装了 [impeccable](https://impeccable.style) 的项目可以用 `npx impeccable detect <files>` 自动化此扫描，仍需在 开发记录.md 里贴输出。
...
3. **检测 DB 凭据 → 反问用户**
   ...
   - 找到凭据 → 反问用户「已检测到数据库凭据，是否现在执行迁移？」选项：
     ...
     2. 只生成 SQL，稍后手动跑
     3. 先看 SQL 内容再决定
   - 未找到凭据（local dev 无配置）→ 生成 SQL + 在 `开发记录.md`「数据库迁移」段显式提醒用户手动在 local / dev / staging / prod 各环境执行

4. **迁移文件写入 开发记录.md「数据库迁移」段**
   ```
   ✅ 数据库迁移（R4.5）：
...
本步生成的迁移文件，5-test 第 4 轮「4.2 数据迁移测试」会再验证一次（在生产数据快照上预演）。所以这里生成的文件必须能被 trace（路径要写进 开发记录.md）。
