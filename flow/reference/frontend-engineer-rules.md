...
### 2.1 颜色一律来自 02a-UI设计.md

- 颜色必须 OKLCH（遗留 hex 必须在 02a-UI设计.md 里显式声明例外）
...
- `@media (prefers-color-scheme)` 处理暗色（除非 02a-UI设计.md 明说不做）
- `@media (prefers-reduced-motion)` 处理动效降级（任何大于 200ms 的动画都要在此 disable）

### 2.4 字体硬禁

**默认禁止直接引用**（不管 02a-UI设计.md 里有没有）：
...
理由：这些字体构成典型 AI-slop 视觉指纹。02a-UI设计.md 的 2a 阶段已经选定别的字体，4-dev 阶段只需要按 02a-UI设计.md 写，不要额外引入。
...
除非：02a-UI设计.md 明确要求，或场景确实非此不可（比如已有 Lottie 资产）。
...
02a-UI设计.md 的"美学北极星"是方向，下面是具体技术手段：
...
- 任务完成 → 回到 `@devflow-kit/flow/prompts/4-dev.md` 步骤 4 self-review
