...
> ⚠️ **强制门控**：本步是**进入任何需求相关阶段的前置门**。未完成本步检测前，AI **禁止**进入以下任一阶段：
> - 0-confirm（需求确认）
> - 1-analysis（需求分析）
> - 2-design（方案设计）
> - 文档解析模式（D.1/D.2/D.3）
>
> **唯一例外**：横向命令（A-architect / A-evolve / I-intel-scan / L-restyle / M-health）不跑本步。

**触发**：路由到任何需求相关阶段之前必跑。

**强制检查点**：在输出路由声明前，必须先输出本步的检测结果：
...
3. 用户未反对该判定
4. **必须提供 verify 命令**：Fast 模式下虽然没有 03-任务拆分.md，但 AI 必须在路由声明中明确写出 verify 命令（如 `npm test -- <file>.test.ts`），否则不允许进入 4-dev
...
**第一门控（强制）**：
- [ ] **入场检测已输出**：已按第二步输出检测结果框（含情况判定、项目类型、上下文文档、入场扫描状态四项）
- [ ] 入场检测结果为"已完成"或"已跳过（用户手动选择）"或"greenfield"

**第二门控（入场检测通过后）**：
- [ ] 已读 `.specs/项目状态.md`（如果存在）
- [ ] 已按表格匹配意图，没有跳过

**第三门控（路由目标 ≠ 0-confirm 时）**：
- [ ] **阶段门已验证**：前置产物已检查；缺失阶段已补跑或用户已显式确认跳过
