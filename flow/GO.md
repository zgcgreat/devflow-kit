...
```markdown
- <20字 → Fast倾向 (+30分)
- 20-100字 → Standard倾向 (+20分)
- >100字 → Strict倾向 (+15分)
```

#### 维度2: 关键词识别
```markdown
Fast关键词: "修复"、"typo"、"文案"、"改颜色"、"调整样式"
Standard关键词: "功能"、"页面"、"API"、"组件"、"接口"
Strict关键词: "支付"、"鉴权"、"迁移"、"重构"、"数据库schema"
```

#### 维度3: 历史相似性（如存在 MODE_HISTORY.md）
```markdown
IF .specs/MODE_HISTORY.md 存在且有≥5条记录:
  - 提取最近5条相似需求
  - 统计各模式使用频率
  - 加权计算推荐分数
ELSE:
...
**场景2: 新功能开发**
```
用户需求: "我想在商品列表页添加搜索功能，支持关键词模糊搜索和排序"
...
3. 预估改动文件数：4-6个（前端组件+后端API+测试）
4. 无高风险关键词

其他选项:
• Fast — 仅当只改UI不涉及API时
• Strict — 如涉及敏感数据或性能要求极高
...
如果 `.specs/MODE_HISTORY.md` 存在，在req完成后自动追加记录：
...
**⚠️ 所有模板都必须遵守此规则**：
- AI 经常跳过模板直接输出，这是最常见的违规行为
- 无论输出哪个产物，都必须先读取对应模板
- 即使是"简单"的上下文.md、项目状态.md，也必须严格按模板输出
...
- 04-开发记录.md：改动摘要、verify 结果
- 05-测试报告.md：测试矩阵、真实命令输出
- 06-代码审查.md：四轮审查记录
