...
**产物输出**：
- 高质量文档：直接生成 `01-需求分析.md`，跳过 `00-需求确认.md`
- 中/低质量文档：先生成 `00-需求确认.md`（已提取部分），澄清后生成 `01-需求分析.md`
...
- 如果用户选择路径 [2]，先走 0-confirm/1-analysis 生成需求文档，再进入 2a-ui-design

**完整路径**：
```
设计稿解析 → 确认需求范围 → 生成 01-需求分析.md → 进入 2a-ui-design
```
...
**产物输出**：
- 路径 [1]：直接生成 `02-方案设计.md`（方案对齐），跳过 `00-需求确认.md` 和 `01-需求分析.md`
- 路径 [2]：先生成 `01-需求分析.md`，再生成 `02-方案设计.md`
- 路径 [3]：走完整流程

**注意**：技术方案解析直接进入 2-design 时，AI 必须在 `02-方案设计.md` 顶部标注：
```markdown
> ⚠️ 本设计由用户上传的技术方案文档直接生成，跳过了 0-confirm 和 1-analysis 阶段。
> 如发现需求不清晰，应暂停并回到 0-confirm 确认。
```
...
**文档解析模式例外**：用户上传高质量文档时（见 D.4 文档质量评估标准），允许：
1. 高质量需求文档 → 跳过 00-需求确认.md，直接生成 01-需求分析.md
2. 高质量需求文档 + 技术方案 → 跳过 00 和 01，直接进入 2-design 或 3-task
3. 设计稿 + 已有需求文档 → 直接进入 2a-ui-design
