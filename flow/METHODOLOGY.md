...
> **你是 AI 助手。这是 flow-kit 的方法论骨架，定义阶段、角色和核心机制。**
>
> **⚠️ 本文件不是路由器。路由和执行步骤见 `flow/GO.md`。**
>
> 本文件定义：阶段定义、文件体系、Agent 角色、核心机制。具体执行按各阶段 `prompts/<n>-*.md` 的指令走。
...
| DEV | TASK 中一项 | fresh context + TDD + diff 边界 verify；默认不自动 commit | 代码 + `开发记录.md` | 否 | 否 |
| TEST | 代码 + AC + 非功能需求 | 风险驱动测试矩阵：功能必跑，性能 / 安全 / 兼容 / 可观测按风险触发 | `05-测试报告.md` | 否 | 否 |
...
| `05-测试报告.md` | 风险驱动测试报告（AC 覆盖 + 必要的性能 / 安全 / 兼容 / 可观测证据）| AI |
...
| `.specs/项目状态.md`| 跨会话状态（当前位置 / 中断任务 / 阻塞 / 决策日志）| AI 维护，人可改 |
