...
> - `.specs/<req-id>/02-方案设计.md`（**需求层**）：单次需求的技术决策、本次需求的接口契约、风险——需求归档后冻结
>
...
- **代价**：多一个运维组件；冷启动慢
- **来源 req**：`add-cache-layer`（`.specs/archive/2026-04-12-add-cache-layer/`）
...
> 详细修订内容见 `.specs/evolve/<date>-EVOLVE.md` 或 `.specs/archive/<arch-req-id>/`。
