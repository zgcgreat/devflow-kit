...
> **本文件 vs 上下文.md vs 02-方案设计.md 的边界**：
> - `上下文.md`（**rules 层**）：技术栈版本、命名约定、既有抽象索引、禁动清单、code-style——AI 实施时**每个需求 都加载**，约 50-200 行
> - `系统架构.md`（**structure 层 · 本文件**）：模块图、跨模块契约、ADR 列表、扩展点、容量边界——**仅 2-design / A-evolve 阶段加载**，约 200-600 行
> - `.devflow-kit/<req-id>/02-方案设计.md`（**需求层**）：单次需求的技术决策、本次需求的接口契约、风险——需求归档后冻结
>
