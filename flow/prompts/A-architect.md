...
> 不属于任何 req，不写 00-需求确认.md。直接产出 / 重构 `.devflow-kit/系统架构.md`。
...
- **首次建立**：项目从未跑过 A-architect，上下文.md 已存在但 系统架构.md 缺失
...
- `@.devflow-kit/上下文.md`（必读 · 技术栈 + 既有抽象 + 禁动清单）
- `@.devflow-kit/系统架构.md`（如有，则是重构跑；如无，则是首跑）
- `@.devflow-kit/需求LOG.md`（看历史 req 演进轨迹）
- `@.devflow-kit/archive/*/02-方案设计.md`（按需 · 重构跑时抽样几份）
- 仓库代码（必读 · `src/` 顶层结构 + `package.json` / 等价依赖文件）
- `@devflow-kit/flow/templates/系统架构.md`（结构模板）
...
✅ 已有 系统架构.md：<是 / 否，N 行，最近修订 YYYY-MM-DD>
...
读 上下文.md「已锁技术决策」段，**每条**升级为 ADR 条目：
...
**首跑**：用 `write_to_file` 创建 `.devflow-kit/系统架构.md`，**⚠️ 强制要求**：必须严格按 `@devflow-kit/flow/templates/系统架构.md` 模板的完整结构输出，按模板 8 段全填（不适用的段可留 N/A 但**不能删段**——后续 A-evolve 要按段 append）。**不得省略或改写任何段落**。

**重构**：

- 先备份：`cp .devflow-kit/系统架构.md .devflow-kit/系统架构.md.bak-<YYYY-MM-DD>`
...
📌 ADR-002 缓存层改用 Redis 是 AI 实施层会感知的决策。
   建议同步到 上下文.md「已锁技术决策」段（让 4-dev 阶段 AI 能读到）。
...
- `.devflow-kit/系统架构.md`（必产，**必须严格按 `@devflow-kit/flow/templates/系统架构.md` 模板完整结构输出**）
- `.devflow-kit/系统架构.md.bak-<date>`（重构跑必产）
- 0~1 次对 `.devflow-kit/上下文.md` 的同步 patch（仅当步骤 7.3 用户选 1）
