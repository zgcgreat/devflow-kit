...
- `@devflow-kit/flow/reference/ui-aesthetics.md`（决策框架）
- `@devflow-kit/flow/reference/ui-anti-patterns.md`（反 AI-slop 清单）
...
#### 1.5.3 写入 02a-UI设计.md

在 `02a-UI设计.md` 的 `## 0. 视觉语汇对齐`（新增段，仅 brownfield 填）贴观察报告 + 用户校准结论。
...
> 写一版完整 02a-UI设计.md 之前，先给 30 秒可看的 v0。方向错了此时换代价最低。
...
- **说"可以 go"** → 进步骤 4，写完整 02a-UI设计.md（保存到 `.devflow-kit/<id>/02a-UI设计.md`）
- **说"某维度偏了"** → 仅改该维，重贴 v0（别改其他维）
- **说"全推了重来"** → 回步骤 1–步骤 2，不是再翻新花样

#### 3.4 进入步骤 4 前写入 02a-UI设计.md

在 02a-UI设计.md 顶部的 `## 1. 美学北极星` 后追加一段 `### v0 确认摸路`，记录：
...
输出 02a-UI设计.md 前，**逐条对照** `ui-anti-patterns.md` 的"强制禁忌"段，确认本设计未命中任何一条。命中的必须改或显式标注例外理由。

## 输出

- `.devflow-kit/<id>/02a-UI设计.md`（**⚠️ 强制要求**：必须严格按 `@devflow-kit/flow/templates/02a-UI设计.md` 模板的完整结构输出。**不得省略或改写任何段落**。）
...
- 步骤 4-5 仍按本 prompt 执行（impeccable 不直接产 02a-UI设计.md）
...
- **R10.4**：02a-UI设计.md 输出前必须 grep `ui-anti-patterns.md` 完成自检
- **R10.5**：brownfield 必走 1.5，视觉语汇对齐报告必须经用户确认后才能进步骤 2
- **R10.6**：v0 草稿必须在写完整 02a-UI设计.md 前给用户确认（除非是极小改动，且用户明说"别 v0 直接写"）
...
```
✅ 2a UI设计 完成
📝 产物：.devflow-kit/<req-id>/02a-UI设计.md
📊 STATE.md 阶段进度已更新：[x] 2a UI设计 → 02a-UI设计.md
...
`@devflow-kit/flow/prompts/3-task.md` —— 拆任务时把 design tokens 物化到代码（CSS variables / theme 文件）作为第一批任务
