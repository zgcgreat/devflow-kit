...
- `@.devflow-kit/<id>/00-需求确认.md`（**必读**，含 0-confirm 阶段已选的「视觉调性」）
- `@.devflow-kit/<id>/01-需求分析.md`
- `@.devflow-kit/<id>/02-方案设计.md`（**必读 `## 0. 技术栈选定` 段**——字体 / UI 库 / 组件库 / 图标库的选择必须基于这个栈）
- `@.devflow-kit/上下文.md`
...
判定信号：`src/` 下已有 `.css` / `.scss` / `.tsx` / `theme` / `design-tokens` 文件，或 `00-需求确认.md` 的视觉调性段标为"沿用现有"。
...
- **目的**：界面解决什么问题？谁用？核心动作？
- **调性**：**从 `00-需求确认.md` 的「视觉调性」段读取已选项**（在 0-confirm 步骤 0.6 已确定）。**不允许在本阶段让用户重选**。
  - 如果 00-需求确认.md 没有该字段（旧项目 / 跳过 0-confirm 直接进 2a）→ 才按 `ui-aesthetics.md` 的「调性」节展示卡片让用户选
  - 如果用户在本阶段主动说"想换调性" → 引导回 0-confirm 改 00-需求确认.md，不在 2a 内部改（避免与原始决策不一致）
...
- **步骤 1**（美学方向）：调性已从 00-需求确认.md 读到，可补 `--domain product` 拿场景模式
...
📊 项目状态.md 阶段进度已更新：[x] 2a UI设计 → 02a-UI设计.md
