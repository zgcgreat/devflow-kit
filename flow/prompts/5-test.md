...
> 本 prompt 先确定风险驱动测试矩阵；具体工具和标准按需查 `@devflow-kit/flow/reference/test-pyramid.md`。
...
**前置**：本任务在 4-dev 步骤 1.7 已生成迁移文件（如未生成，回退到 dev 阶段——R4.5 违规）。在 开发记录.md「数据库迁移」段 trace 文件路径，本步对这些文件做端到端验证。

- [ ] 迁移文件路径已 trace（来自 `<task-id>-开发记录.md`「数据库迁移」段）
...
- **R11.4**：不允许跳过风险触发的测试轮次而没有显式理由（在范围声明里写明）
...
- 任何轮次发现 🔴 问题 → 回到 `@devflow-kit/flow/prompts/4-dev.md` 修复（产 fix 任务）
- 全部通过 → `@devflow-kit/flow/prompts/6-review.md`（review 阶段会检查风险触发的测试是否都有证据）
