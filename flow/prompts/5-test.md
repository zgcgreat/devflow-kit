...
> 每轮按项目类型可裁剪。本 prompt 决定**做哪几轮**；具体怎么做查 `@devflow-kit/flow/reference/test-pyramid.md`。
...
在 `05-测试报告.md` 开头**显式输出适用矩阵**，对照 `test-pyramid.md` 末尾的「适用矩阵」表，按项目类型决定每轮是「✅ 必跑 / ⚠️ 部分 / ❌ 跳过」。
...
- **R11.4**（新）：不允许跳过任何轮次而没有显式理由（在范围声明里写明）
- **R11.5**（新）：性能 / 安全 / 兼容轮次的"通过 / 失败"必须基于**可量化指标**或工具输出，不允许"看起来没问题"
- AC 覆盖优先于行覆盖
...
> **执行阶段**：测试是执行阶段，用户不回复默认 Y，可自动进入下一阶段。

## 触发下一步

- 任何轮次发现 🔴 问题 → 回到 `@devflow-kit/flow/prompts/4-dev.md` 修复（产 fix 任务）
- 全部通过 → `@devflow-kit/flow/prompts/6-review.md`（review 阶段会再检查 5 轮都做了）
