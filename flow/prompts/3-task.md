...
模式判定：
- Fast 模式：直接进入开发阶段
- Standard/Strict 模式：询问用户是否生成详细实施计划（03a-实施计划.md）

➡️ 下一步：
- Fast：进入开发阶段
- Standard/Strict + 用户确认：生成 03a-实施计划.md → 进入开发阶段
```

> **执行阶段**：任务拆分是执行阶段，无阻塞且下一步明确时可继续进入下一阶段。
>
> **Standard/Strict 模式增强**：在任务拆分完成后，如用户要求详细设计，应生成 `03a-实施计划.md`，为每个任务提供：
> - 修改点清单（文件路径 + 操作类型）
> - 伪代码/关键逻辑说明
> - 影响范围分析（直接/间接影响 + 风险评估）
> - 验证命令和预期输出
...
### Fast 模式
`@devflow-kit/flow/prompts/4-dev.md`（按执行顺序逐个执行）

### Standard/Strict 模式（可选详细设计）
1. 询问用户：“是否生成详细实施计划（03a-实施计划.md）？”
2. 如用户确认：
