...
- `@.devflow-kit/<req-id>/01-analysis.md`（或 `01-需求分析-Delta.md`）
...
| 模式 | 模板 | 说明 |
|---|---|---|
| 新需求 | `@devflow-kit/flow/templates/02-design.md` | 完整格式，所有字段必填 |
...
- `01-analysis.md`（或 `01-需求分析-Delta.md`）中明确标注了「本次为增量需求」
...
> 以后用户跑 `@devflow-kit/flow/prompts/A-evolve.md` 会扫这段批量同步到 `CONTEXT.md`。
...
按 `@devflow-kit/flow/templates/02-design.md` § 9 的五个子段（抽象 / 决策 / 契约 / 依赖 / 禁动清单）填写。不适用的子段不写也可（留一句"N/A"）。
