...
**⚠️ 发现 Critical 问题后，必须一次性列出所有问题，然后统一询问**：
...
│     AC-02 的边界条件未测试                                   │
│                                                              │
│  3. [Security] src/api/users.ts:15                           │
│     SQL 注入风险：用户输入直接拼接                            │
├─────────────────────────────────────────────────────────────┤
│  请选择处理方式：                                             │
...
| 用户输入 | AI 行为 |
|----------|---------|
| `fix 1,2` | 为问题 1、2 生成 fix 任务，问题 3 继续等待处理 |
| `fix all` | 为所有问题生成 fix 任务 |
| `accept 3` | 输出"⚠️ 确认接受问题 3 的风险？(y/n)"，等待确认后记录"已知接受（用户确认：YYYY-MM-DD）" |
| `discuss 1` | 输出问题 1 的完整 Symptom/Source/Consequence/Remedy，然后继续等待 |
| `details` | 输出所有问题的完整信息，然后继续等待 |
