...
使用 `@devflow-kit/flow/templates/健康巡检.md` 模板，**⚠️ 强制要求**：必须严格按模板完整结构输出，写入 `.devflow-kit/health/<YYYY-MM-DD>-HEALTH.md`。**不得省略或改写任何段落**。
...
- 🟢 Monitored → 追加到 `.devflow-kit/经验总结.md`（用 `观察` 类型 entry）
...
- `.devflow-kit/health/<YYYY-MM-DD>-HEALTH.md`（必产，**必须严格按 `@devflow-kit/flow/templates/健康巡检.md` 模板完整结构输出**）
- 0~N 个新需求 提案（仅 Critical 项）
- CONTEXT.md / 经验总结.md 的追加（自动）
