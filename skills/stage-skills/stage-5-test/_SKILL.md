...
- `.devflow-kit/<req-id>/04-dev-log.md` + 各 `<task-id>-dev-log.md`
...
| lessons-learned.md | ✅/❌ | 🟢 可选 | 测试教训、常见bug模式 |
...
❌ 未检测到 lessons-learned.md → 使用通用测试最佳实践
```
...
2. **03-tasks.md 缺失**：
   ...
   → 降级方案：从04-dev-log推断改动文件
   → 或询问用户：“请确认本次改动的文件范围”
   ```

3. **lessons-learned.md 缺失**：
   ```
   ⚠️ 警告：缺少lessons-learned.md，无法参考历史测试教训
...
```python
# 伪代码示例
read_file("templates/05-test-report.md")
```
...
- ⏳ **已使用 read_file 读取模板文件** `templates/05-test-report.md`
- ⏳ **已从模板提取必填段落清单**（6个段落）
...
- 用户确认测试通过 → 加载 `skills/stage-skills/stage-6-review/_SKILL.md`
