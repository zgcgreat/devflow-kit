# ARCHITECTURE — 项目级system-architecture（活文档）

- **维护者**：`prompts/A-architect.md`（首次 / 重构）+ `prompts/A-evolve.md`（增量同步 ADR）
- **首次创建**：<YYYY-MM-DD>
- **最近修订**：<YYYY-MM-DD>（来自 A-evolve <YYYY-MM-DD> · 详见 `.devflow-kit/evolve/<date>-EVOLVE.md`）
- **当前 ADR 编号最大值**：ADR-007

> **本文件 vs CONTEXT.md vs 02-design.md 的边界**：
> - `CONTEXT.md`（**rules 层**）：技术栈版本、命名约定、既有抽象索引、禁动清单、code-style——AI 实施时**每个需求 都加载**，约 50-200 行
> - `system-architecture.md`（**structure 层 · 本文件**）：模块图、跨模块契约、ADR 列表、扩展点、容量边界——**仅 2-design / A-evolve 阶段加载**，约 200-600 行
> - `.devflow-kit/<req-id>/02-design.md`（**需求层**）：单次需求的技术决策、本次需求的接口契约、风险——需求归档后冻结
>
> **三层职责无重叠**：CONTEXT 回答"AI 写代码时该遵守什么"，ARCHITECTURE 回答"系统是怎么搭起来的"，DESIGN 回答"这次 req 怎么做"。
...
> 任何会影响这些指标的 req，2-design 阶段必须显式评估。
...
如果新需求 需要破例，必须在 DESIGN § 0.5 显式声明并升级 ADR。

---

## 3. ADR 列表（Architecture Decision Records · 不可逆决策清单）

> 每条 ADR 是项目级"以后都这么干"的取舍。低成本可逆决策不进这表（留在 需求级 DESIGN）。
...
- **代价**：多一个运维组件；冷启动慢
- **来源 req**：`add-cache-layer`（`.devflow-kit/archive/2026-04-12-add-cache-layer/`）
...
- **理由**：…
- **代价**：…
- **来源需求**：<req-id 或 (initial)>
- **推翻成本**：低 / 中 / 高

> **新增 ADR 走 A-evolve**——单 需求的「DESIGN § 9.2 项目级技术决策」段经用户 review 后，由 A-evolve 转入此处。
...
> 这里记**项目级**技术债。需求级的债走 SUMMARY → LESSONS。

| 债 | 影响 | 优先级 | 触发条件（什么时候必须还）|
|---|---|---|---|
| <例：legacy `cache-old.ts` 仍被 3 个旧路由使用> | 缓存策略不一致 | 中 | 那 3 个路由下次有 req 时必须替换 |
...
> 详细修订内容见 `.devflow-kit/evolve/<date>-EVOLVE.md` 或 `.devflow-kit/archive/<arch-req-id>/`。
